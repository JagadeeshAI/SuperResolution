...
class Restormer(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        # print('before upscaling output', out_dec_level1.size())
        out_dec_level1 = self.output_upscale(out_dec_level1)

        inp_img = F.interpolate(
            inp_img, scale_factor=2, mode="bilinear", align_corners=False
        )

        # print('upscaled output', out_dec_level1.size())
        #### For Dual-Pixel Defocus Deblurring Task ####
        if self.dual_pixel_task:
...
if __name__ == "__main__":
    ...
    # Initialize the Restormer model for super-resolution
    model = Restormer(
        inp_channels=4,  # Set to 4 for the specified input shape
        out_channels=4,
        dim=16,
    ...
    # Test with input size [1, 4, 182, 274]
    test_input = torch.randn(1, 4, 128, 128)

    # Memory after creating input
    if torch.cuda.is_available():
        print(
            f"GPU Memory After Input: {torch.cuda.memory_allocated(Config.device)/1024/1024:.2f} MB"
        )
    ...
    if torch.cuda.is_available():
        print(
            f"GPU Memory After Forward Pass: {torch.cuda.memory_allocated(Config.device)/1024/1024:.2f} MB"
        )

    # Check output shape - should be doubled spatial dimensions
    expected_h, expected_w = test_input.shape[2] * 2, test_input.shape[3] * 2
    print(f"Input: {test_input.shape}, Output: {output.shape}")
    print(f"Expected output shape: (1, 4, {expected_h}, {expected_w})")
