...
def extract_and_save_patches(
    ...
    if existing_patches:
        ...
        user_response = input("Do you want to delete existing patches and recreate them? (y/n): ")
    ...
    for file_idx, path in enumerate(tqdm(data_paths)):
        try:
            ...
            with np.load(path) as data:
                ...
                hr_patches, total_patches_count = extract_patches(hr_img, patch_size=512)
    ...
    print(f"Finished processing {len(data_paths)} images. Total patches extracted: {total_patches}")
